...
class Marine(AttackUnit):
    ...
    def stimpack(self):
        if self.hp > 10:
            ...
            print("{0} : 체략이 부족하여 스팀팩을 사용하지 않습니다.".format(self.name))
...
class Wraith(FlyableAttack):
    def __init__(self):
        ...
        self.clocked = False # 클로킹 모드 (OFF 상태)
...
for unit in attack_units:
    if isinstance(unit, Marine): # unit 이 클래스 Marine의 instance라면..
        unit.stimpack()
...
# 전군 피해
for unit in attack_units:
    unit.damaged(randint(5,20)) # 공격은 랜덤으로 받음 (5이상 20이하)
